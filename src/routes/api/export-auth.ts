    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          // Hanya akun yang diberi app_metadata.export_access=true yang boleh export.
          // app_metadata tidak bisa diubah oleh user biasa dan cocok untuk authorization.
          if (user.app_metadata?.export_access !== true) {
            return Response.json({ error: "Akun ini tidak punya akses export." }, { status: 403 });
          }

          const url = new URL(request.url);
          const jenis = url.searchParams.get("jenis") as Jenis | null;
          if (!jenis || !["rekap", "siswa", "chat", "quiz", "anonim"].includes(jenis)) {
            return Response.json({ error: "Jenis export tidak dikenal." }, { status: 400 });
          }
          ...
          if (jenis === "siswa") {
            return csvResponse(
              ...
              csvFile(
                ...
                students.map((s) => [s.nama, s.kelas, s.sekolah, s.consent_at, s.created_at]),
              ),
            );
          }
          ...
          if (jenis === "chat") {
            return csvResponse(
              ...
              csvFile(
                ["created_at", "nama", "kelas", "sekolah", "materi_id", "sumber", "role", "content"],
                identified.map((c) => {
                  const s = c.student_id ? byId.get(c.student_id) : undefined;
                  return [c.created_at, s?.nama ?? "", s?.kelas ?? "", s?.sekolah ?? "", c.materi_id, c.sumber, c.role, c.content];
                }),
              ),
            );
          }
          ...
          if (jenis === "anonim") {
            return csvResponse(
              ...
              csvFile(
                ...
                anonymous.map((c) => [c.created_at, c.materi_id, c.sumber, c.role, c.content]),
              ),
            );
          }
          ...
          if (jenis === "quiz") {
            return csvResponse(
              ...
              csvFile(
                ["finished_at", "nama", "kelas", "sekolah", "materi_id", "score", "total", "started_at", "answers"],
                attempts.map((a) => {
                  const s = byId.get(a.student_id);
                  return [a.finished_at, s?.nama ?? "", s?.kelas ?? "", s?.sekolah ?? "", a.materi_id, a.score, a.total, a.started_at, JSON.stringify(a.answers ?? [])];
                }),
              ),
            );
          }
          ...
          for (const c of userChats) {
            const arr = chatsByStudent.get(c.student_id!) ?? [];
            arr.push(c);
            chatsByStudent.set(c.student_id!, arr);
          }
          ...
          const header = [
            "nama", "kelas", "sekolah", "consent_at",
            ...ordered.map((m) => `chat_${m.id}`),
            "chat_total", "materi_diskusi", "quiz_skor", "quiz_total", "quiz_tanggal", "quiz_attempts",
          ];
          const rows = students.map((s) => {
            const list = chatsByStudent.get(s.id) ?? [];
            const perMateri = ordered.map((m) => list.filter((c) => c.materi_id === m.id).length);
            const finals = finalsByStudent.get(s.id) ?? [];
            const latest = finals[finals.length - 1];
            return [s.nama, s.kelas, s.sekolah, s.consent_at, ...perMateri, list.length, new Set(list.map((c) => c.materi_id)).size, latest?.score ?? "", latest?.total ?? "", latest?.finished_at ?? "", finals.length];
          });
          ...
          return Response.json(
            { error: err instanceof Error ? err.message : "Gagal membuat export." },
            { status: 500 },
          );
        }
      },
